<section class="not-format">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-lg lg:text-2xl font-bold text-gray-900 dark:text-white">Discussion {{ $this->comments->count() }}
        </h2>
    </div>
    {{-- <form class="mb-6"> --}}
    <div
        ...
        <textarea wire:model="comment" rows="6"
            ...
            placeholder="Escribe un comentario, o haz una pregunta..." required></textarea>
    </div>
    ...
    @if (session()->has('success'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
            class="p-4 mt-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800 transition-opacity duration-500"
    ...
    @if (session()->has('error'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
            class="p-4 mt-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800 transition-opacity duration-500"
            x-transition:enter="opacity-0" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="opacity-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
            {{ session('error') }}
        </div>
    ...
    @forelse ($this->comments as $comment)
        <article class="p-6 mb-6 text-base bg-white rounded-lg dark:bg-gray-900">
            <footer class="flex justify-between items-center mb-2">
                ...
                <div x-data="{ commentSettings: false }">
                    <button @click="commentSettings = !commentSettings" @click.away="commentSettings = false"
                    ...
                    <!-- Dropdown menu -->
                    <div x-show="commentSettings" x-cloak
                            ...
                            @if ($comment->author->id === Auth::user()->id)
                                <li>
                                    <button wire:click="deleteComment({{ $comment->id }})"
                                        class="block w-full text-left px-4 py-2 hover:underline hover:bg-gray-100 dark:hover:bg-gray-700">
                                        Eliminar
                                    </button>
                                </li>
                            @endif
                            <li>
                                <button wire:click="$dispatch('openModal', { component: 'post-comment-report', arguments: { commentId: {{ $comment->id }} }})"
                                        class="block w-full text-left text-red-500 hover:underline px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    Reportar
                                </button>
                            </li>
                        </ul>
    ...
    @endforelse
{{--     <livewire:post-comment-report :key="'report' . $comment->id" :commentId="$comment->id" /> --}}
    <div class="my-2">
        {{ $this->comments->links() }}
    </div>
